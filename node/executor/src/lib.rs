...
mod tests {
	...
	use primitives::{
		Blake2Hasher, NeverNativeValue, NativeOrEncoded, map, storage::well_known_keys,
	};
...
"#;
	...
	fn wasm_big_block_import_fails() {
		...
		let (result, _) = executor().call::<_, NeverNativeValue, fn() -> _>(
			&mut t,
			"Core_execute_block",
	...
	fn panic_execution_gives_error() {
		...
		let (r, _) = executor().call::<_, NeverNativeValue, fn() -> _>(
			&mut t,
			"Core_initialize_block",
			&vec![].and(&from_block_number(1u32)),
			false,
