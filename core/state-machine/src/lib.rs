...
use std::{fmt, result, marker::PhantomData, panic::UnwindSafe};
use log::{warn, trace};
use hash_db::Hasher;
...
pub fn prove_read<B, H, I>(
	mut backend: B,
	keys: I,
) -> Result<(Vec<Option<Vec<u8>>>, StorageProof), Box<dyn Error>>
where
	B: Backend<H>,
	H: Hasher<Out=H256>,
	H::Out: Ord,
	...
	I::Item: AsRef<[u8]>,
{
	...
	let values = keys.into_iter()
		...
		.collect::<Result<_, _>>()?;
	Ok((values, proving_backend.extract_proof()))
}
...
pub fn read_proof_check<H, I, K, V>(
	root: H::Out,
	proof: StorageProof,
	items: I,
	...
	V: AsRef<[u8]>,
{
	...
	for (key, expected_value) in items {
		let actual_value = read_proof_check_on_proving_backend(&proving_backend, key.as_ref())?;
		if actual_value.as_ref().map(AsRef::as_ref) != expected_value.as_ref().map(AsRef::as_ref) {
			return Err(Box::new("incorrect value in read proof"));
		}
	}
	Ok(())
}
...
pub fn read_child_proof_check<H, I, K, V>(
	root: H::Out,
	proof: StorageProof,
	storage_key: &[u8],
	items: I,
	...
	V: AsRef<[u8]>,
{
	let proving_backend = create_proof_check_backend::<H>(root, proof)?;
	for (key, expected_value) in items {
		let actual_value = read_child_proof_check_on_proving_backend(
			&proving_backend,
			storage_key,
			key.as_ref(),
		)?;
		if actual_value.as_ref().map(AsRef::as_ref) != expected_value.as_ref().map(AsRef::as_ref) {
			return Err(Box::new("incorrect value in read child proof"));
		}
	}
	Ok(())
}
...
mod tests {
	...
	fn prove_read_and_proof_check_works() {
		...
		let remote_backend = trie_backend::tests::test_trie();
		let remote_root = remote_backend.storage_root(::std::iter::empty()).0;
		let (values, remote_proof) = prove_read(remote_backend, &[b"value2"]).unwrap();
		// check values
		assert_eq!(values, vec![Some(vec![24])]);
 		// check proof locally
		let local_result1 = read_proof_check::<Blake2Hasher, _, _, _>(
			remote_root,
			remote_proof.clone(),
			vec![(b"value2", values[0].as_ref())],
		);
		let local_result2 = read_proof_check::<Blake2Hasher, _, _, _>(
			remote_root,
			remote_proof.clone(),
			vec![(&[0xff], Some(&[0xff]))],
		);
 		// check that results are correct
		assert!(local_result1.is_ok());
		assert!(local_result2.is_err());
		// on child trie
		let remote_backend = trie_backend::tests::test_trie();
		let remote_root = remote_backend.storage_root(::std::iter::empty()).0;
		let (values, remote_proof) = prove_child_read(
			remote_backend,
			b":child_storage:default:sub1",
			&[b"value3", b"value2"],
		).unwrap();
		assert_eq!(values, vec![Some(vec![142]), None]);
		let local_result1 = read_child_proof_check::<Blake2Hasher, _, _, _>(
			remote_root,
			remote_proof.clone(),
			b":child_storage:default:sub1",
			vec![(b"value3", values[0].as_ref())],
		);
		let local_result2 = read_child_proof_check::<Blake2Hasher, _, _, _>(
			remote_root,
			remote_proof.clone(),
			b":child_storage:default:sub1",
			vec![(b"value2", values[1].as_ref())],
		);
		assert!(local_result1.is_ok());
		assert!(local_result2.is_ok());
	}
